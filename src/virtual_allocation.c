...
pt_entry *setup_pt(virtual_addr virt, uint64_t base) {
   ...
   l3 = (pt_entry*)(l4[virt.l4_ndx].base_addr << 12);
   ...
   l2 = (pt_entry *) (l3[virt.l3_ndx].base_addr << 12);
   ...
   l1 = (pt_entry*)(l2[virt.l2_ndx].base_addr <<12);
...
void *MMU_alloc_page() {
   ...
   return (void *) (heap_bottom - PAGE_SIZE);
}
...
void MMU_free_page() {
   ...
   MMU_pf_free((void *)(l1[virt.l1_ndx].base_addr << 12));
   l1[virt.l1_ndx].p = 0;
...
void setup_id_map(uint64_t phy_mem_size) {
   ...
   for(i=0; i < phy_mem_size/PAGE_SIZE; i++){
      j.i = i * PAGE_SIZE;
      connect_addr(j.va, j.i, (uint64_t)l4);
   }
